 ...
 * Opening fence: 3+ colons plus optional Pandoc attributes and title text.
 *
 * Accepting trailing title text keeps theorem/proof authoring forgiving while
 * preserving the canonical title="..." attribute form generated by Blueprint.
 * Self-closing single-line divs are not canonical fenced-div blocks.
 */
function parseOpeningFence(
  ...
  let titleFrom = cursorAfterAttr;
  while (titleFrom < text.length && isSpaceTab(text.charCodeAt(titleFrom))) {
    titleFrom++;
  }
  ...
  if (titleFrom < titleTo) {
    const titleText = text.slice(titleFrom, titleTo);
    const attrText = attrFrom >= 0 ? text.slice(attrFrom, attrTo) : "";
    if (
      titleText.endsWith(":::")
      || !attrText.startsWith("{")
      || /\btitle\s*=/.test(attrText)
    ) {
      return undefined;
    }
  }
